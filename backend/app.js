...
app.get('/api/advertisedColleges', async (req, res) => {
    try {
      const advertisedColleges = await AdvertisedCollege.find(); // Assuming AdvertisedCollege is your MongoDB model
      res.json(advertisedColleges);
    } catch (error) {
      res.status(500).json({ message: 'Failed to fetch advertised colleges.' });
    }
});
